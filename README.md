...
- Index: Start Menu apps, files in Desktop / Downloads / Documents (top level
  plus one nested level, capped at 400 entries per folder), nine system toggles.
...
3. Either set a `TYPESAFE_API_KEY` environment variable or paste the key into
   ...
   launcher.
4. Type `jv` plus your query, for example `jv the pdf I just downloaded`.
...
Publish is tag-driven, same as the IPDetails plugin:
